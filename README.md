...
The system applies Pareto sorting mechanics to prioritize inventory items based on financial impact. Given the set of items , for each item , the total expenditure value  is computed as:
Items are sorted in descending order of . The cumulative percentage of total expenditure is calculated as:
Using configurable threshold boundaries (defaults: , ), the system categorizes inventory:
...
SimpleForecastStrategy (Baseline): Computes the basic monthly historical average consumption, scaling it by an expected annual growth projection (default: , representing  annual growth):
HoltWintersForecastStrategy: Applies triple exponential smoothing models via statsmodels with additive trend and seasonal components. This strategy requires at least two complete seasonal periods ( months of historical data) to run.
...
The system dynamically runs K-Means clustering across a range of clusters (). It evaluates each configuration by computing the silhouette score:
...
If the optimal silhouette score drops below the confidence threshold (), the system rejects the K-Means boundaries. It automatically falls back to a deterministic Quantile Scoring Matrix:
...
Initialize the Environment: Create a localized virtual environment and install all pinned dependencies using uv:
uv venv
...
Generate Required Directories: Create target directory pathways for raw data, models, logs, and outputs:
...
Configure Output: Select the output directory and specify the report filename. Enter the expected annual growth percentage (defaults to ).
